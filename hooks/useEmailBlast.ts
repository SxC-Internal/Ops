...
export function useEmailBlast(user: User) {
  ...
  const opsRole = useMemo(() => getOpsRole(user), [user]);
  const isManager = useMemo(() => isOpsManager(user), [user]);
  ...
  const resetComposer = useCallback(() => {
    ...
    setComposerSenderEmail((prev) => prev);
  ...
  const insertImageIntoComposerBody = useCallback((attachmentId: string) => {
    const attachment = composerAttachments.find((a) => a.id === attachmentId);
  ...
  const submitForApproval = useCallback(() => {
    if (!composerSubject.trim() || !composerBody.trim() || composerRecipients.length === 0) return;
  ...
  const archiveBlast = useCallback((blastId: string) => {
    void (async () => {
      try {
        ...
        setActionError(message);
        addToast({ type: "error", title: "Archive Failed", message });
      }
    })();
  }, [user, refreshBlasts, addToast]);
    ...
    void (async () => {
      try {
        ...
        await createBlast(user, {
          ...
          recipients: getRecipientsForBlast(blast.id).map((r) => r.email),
        ...
        setActionError(message);
        addToast({ type: "error", title: "Duplicate Failed", message });
      }
    })();
