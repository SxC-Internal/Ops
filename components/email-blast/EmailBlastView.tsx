...
import BlastComposerForm from '@/components/email-blast/email/BlastComposerForm';
import BlastHistoryTable from '@/components/email-blast/email/BlastHistoryTable';
import PendingApprovalsPanel from '@/components/email-blast/email/PendingApprovalsPanel';
...
const EmailBlastView: React.FC<EmailBlastViewProps> = ({ user, theme }) => {
  const {
    allBlasts,
    pendingBlasts,
    isLoading,
  ...
  const roleBadgeLabel = opsRole === 'manager' ? 'Ops Manager' : 'Ops Associate';
  const roleBadgeCls =
    opsRole === 'manager'
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 flex items-center gap-2">
            <Mail size={15} />
            Compose and manage email blasts.
          </p>
        </div>
