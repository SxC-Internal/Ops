...
import { archiveEmailBlast, blastIdSchema } from "@/lib/server/email-blast-service";
import {
    apiError,
    apiSuccess,
...
export async function POST(
    ...
    try {
        ...
        const blast = await archiveEmailBlast(parsed.data.blastId, user);
        ...
        return apiError(getPublicErrorMessage(error, status, "Failed to archive email blast"), status);
    }
}
