...
export async function DELETE(
    ...
    try {
        ...
        if (!parsedBlast.success) {
            return apiError(parsedBlast.error.issues.map((i) => i.message).join("; "), 422);
        }
        ...
        return apiError(getPublicErrorMessage(error, status, "Failed to delete attachment"), status);
    }
}
