...
import { approveEmailBlast, blastIdSchema } from "@/lib/server/email-blast-service";
import {
    apiError,
    apiSuccess,
...
export async function POST(
    ...
    try {
        ...
        const blast = await approveEmailBlast(parsed.data.blastId, user);
        ...
        return apiError(getPublicErrorMessage(error, status, "Failed to approve email blast"), status);
    }
}
